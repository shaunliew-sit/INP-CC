...
class SWiGHOIDetection(CocoDetection):
    ...
    def __getitem__(self, idx: int):
        ...
        target = {
            ...
            "hois": annos["hois"],
            # "filename": filename,
            "fingerprint": torch.tensor(self.filename2embedding[filename.split("/")[-1]]),
        }

        if self.transforms is not None:
            image, target = self.transforms(image, target)
        
        mask_region_hw = boxes_to_masks(target["boxes"], 14, 14)
        target["mask_region_hw"] = mask_region_hw
        return image, target
...
def boxes_to_masks(boxes, height, width):
    # 创建一个全为零的mask tensor
    masks = torch.zeros((boxes.size(0), height, width), dtype=torch.float32)
